...
const (
	...
	secondaryAwsCredentialJSONFile        = "SECONDARY_AWS_APPLICATION_CREDENTIALS_JSON"
	secondaryAwsCredentialDirectory       = "SECONDARY_AWS_APPLICATION_CREDENTIALS"
)
...
type S3SnapStore struct {
	// Interface fields first
	client s3api.Client
	// 8-byte fields
	minChunkSize            int64
	maxParallelChunkUploads uint
	// String fields (pointer-sized on 64-bit systems)
	prefix  string
	bucket  string
	tempDir string
	// identifier is an optional field to identify the S3 snapstore instance in logs.
	identifier string
...
func NewS3SnapStore(config *brtypes.SnapstoreConfig, identifier string) (*S3SnapStore, error) {
	...
	// For secondary storage, check SECONDARY_STORAGE_CONTAINER if bucket is empty
	if bucket == "" && getEnvPrefixStringForConfig(config) == secondaryPrefixString {
		if secondaryBucket := os.Getenv("SECONDARY_STORAGE_CONTAINER"); secondaryBucket != "" {
			bucket = secondaryBucket
		}
	}

	cli := s3.NewFromConfig(cfg, cliOpts...)
	return NewS3FromClient(bucket, config.Prefix, config.TempDir, config.MaxParallelChunkUploads, config.MinChunkSize, cli, sseCreds, identifier), nil
}

func getConfigOpts(prefixString string) ([]func(*awsconfig.LoadOptions) error, []func(*s3.Options), SSECredentials, error) {
	// First check for JSON file with the given prefix
