...
func NewECSSnapStore(config *brtypes.SnapstoreConfig, identifier string) (*S3SnapStore, error) {
	ao, err := ecsAuthOptionsFromEnv()
	if err != nil {
		return nil, err
	}
	return newGenericS3FromAuthOpt(config.Container, config.Prefix, config.TempDir, config.MaxParallelChunkUploads, config.MinChunkSize, ao, identifier)
}
