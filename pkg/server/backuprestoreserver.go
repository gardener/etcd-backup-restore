...
func (b *BackupRestoreServer) runEtcdProbeLoopWithSnapshotter(ctx, ssrCtx context.Context, handler *HTTPHandler, ssr *snapshotter.Snapshotter) {
	...
	for {
		...
		if ssr.PrevFullSnapshot != nil && time.Since(ssr.PrevFullSnapshot.CreatedOn).Hours() <= recentFullSnapshotPeriodInHours {
			...
			if err == nil {
				if _, err := ssr.TakeDeltaSnapshot(ssrCtx); err != nil {
		...
		if !initialDeltaSnapshotTaken {
			...
			if _, err := ssr.TakeFullSnapshotAndResetTimer(ssrCtx); err != nil {
		...
		if err := ssr.Run(ssrCtx, startWithFullSnapshot); err != nil {
