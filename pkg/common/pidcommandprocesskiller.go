...
func (k *pidCommandProcessKiller) Kill(ctx context.Context) (bool, error) {
	...
	k.logger.Infof("Determining pid of process %q by executing command `%s`...", k.processName, k.pidCommand)
	out, err := k.commandFactory.NewCommand(ctx, "sh", "-c", k.pidCommand).Output()
	if err != nil {
		return false, fmt.Errorf("could not determine pid of process %q: %w", k.processName, err)
	}
	...
	k.logger.Infof("Killing process %q with pid %s...", k.processName, pid)
	if err := k.commandFactory.NewCommand(ctx, "sh", "-c", fmt.Sprintf("kill %s", pid)).Run(); err != nil {
		return false, fmt.Errorf("could not kill process %q with pid %s: %w", k.processName, pid, err)
	}
	return true, nil
}
