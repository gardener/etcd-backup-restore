...
# cluster_deletion_trap deletes the kind cluster upon receiving a trap signal in the create_or_target_cluster() or setup() function
function cluster_deletion_trap() {
...
Please make sure the following environment variables are set:
    ...
    GOOGLE_EMULATOR_HOST              Host of the fake-gcs service. ( required only for testing with fake-gcs)
EOM
  return 1
}
...
Please make sure the following environment variables are set:
    ...
    AZURITE_DOMAIN                    Host of the Azurite service. ( required only for testing with Azurite)
...
# build_and_load builds the etcdbr image and loads it into the kind cluster
build_and_load() {
...
function create_or_target_cluster() {
  trap cluster_deletion_trap INT TERM
  if containsElement $STEPS "setup"; then
