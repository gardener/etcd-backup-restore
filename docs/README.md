# Documentation Index

## Deployment

* [Getting started](deployment/getting_started.md)

## Development

* [Setting up a local development environment](development/local_setup.md)
* [Testing and Dependency Management](development/testing_and_dependencies.md)
* [Tests](development/tests.md)
* [Adding support for a new object store provider](development/new_cp_support.md)

## Concepts

* [Anti-rejoin guard](concepts/anti-rejoin-guard.md)

## Design and Proposals

* [Core design](proposals/design.md)
* [Etcd data validation](proposals/validation.md)
* [Data restoration](proposals/restoration.md)
* [Leader Election](operations/leader_election.md)
* [High watch events ingress rate issue](proposals/high_watch_event_ingress_rate.md)

## Operations

* [Manual restoration](operations/manual_restoration.md)
* [Generating SSL certificates](operations/generating_ssl_certificates.md)

## Monitoring

* [Metrics](operations/metrics.md)
